...
class MobileAppApiController extends Controller
{
    ...
    public function category($storeId){
        ...
        if($store) {
            ...
            $category_id = '4,6,7,8,9,32,10,11,55,56,57,61,64';
            ...
            $all_pizza = Product::published()->with(['addons', 'ingredients'])
                ...
                ->whereIn('products.device_dependency',['app','both'])
                ->select('categories.id as category_id', 'categories.name as category_name', 'products.*')
                ->orderByRaw("FIELD(category_id,6,56,4,7,8,9,10,11,32,55,57,61,64)")
    ...
    public function productByCategoryName($name,$storeId){
        ...
        if($store) {
            if ($name == 'pizza') {
                ...
                // }
                $category_id = '4,6,7,8,9,32,10,11,55,56,57,61,75,64,15';
                $category_ids = explode(',', $category_id);
                $products = Product::published()->with(['addons', 'ingredients'])
                    ...
                    ->orderByRaw("FIELD(category_id,61,75,6,4,56,7,8,9,10,11,32,55,57,61,'15')")
                    ->orderBy('products.price_medium','asc')
                    ->get();
...
//        if($dealNotProducts==true){
                if (strcasecmp($deal->title, 'wow') == 0) {     //  Pizza Size can choose
                    ...
                    if (Str::contains($deal->title, 'Sundays') || Str::contains($deal->title, 'BOGO')) {// Only First Pizza Price will applicable
                        ...
                        return Response::json(compact('deal_type', 'deal', 'favorite_lien_pizza', 'supreme_lien_pizza', 'super_supreme_pizza', 'veg_delight', 'cheesy_bites', 'golden_surprise', 'golden_surprise_fav', 'cheesy_bites_fav', 'thin_crust', 'crown', 'kabab_crust','battle_of_beef','new_line'));
...
//            return view('front.deals.deal-products', compact(['deal','products']));
    ...
    public function orderSubmit(Request $request)
    {
        ...
        $customer_email = $this->aesDecrypt($request->customer_email);
        ...
        if(isset($request->coupon) && $request->coupon!=null){
            $coupon_code  = $this->aesDecrypt($request->coupon);
        }else{
           $coupon_code = ' '; 
        }
        $order = new Order(); 
...
//&2")."&sms[1][2]=123456790&sid=$sid";
    ...
    public function updateSSLPaymentStatus(Request $request){
        ...
        $pgwadata =  $request->pgw_data;
        // $pgwadata = json_decode($pgwadata);
        $ssl_payment = SslPayment::where('order_id',$order_id)->first();
        if(!$ssl_payment){
        $ssl = new SslPayment();
        $ssl->order_id = $request->order_id;
        $ssl->amount = $pgwadata['amount'];
        $ssl->bank_tran_id = $pgwadata['bank_tran_id'];
        $ssl->card_brand = $pgwadata['card_brand'];
        $ssl->card_issuer = $pgwadata['card_issuer'];
    ...
    //         $ssl->save();
    //     }else{
    //         $ssl_payment->ipn_grab_dec = $pgwadata;
    //         $ssl_payment->save();
    //     }
    //     $result = $this->validPaymentWithIPN($pgwadata['val_id']);
    //     $order = Order::find($order_id);
    //     $payment = SslPayment::where('order_id',$order_id)->first();
    //     $payment->status = $result->status;
    //     $payment->save();
    //     if($result->status == 'VALID' || $result->status=='VALIDATED') {
    //         $order_total = $order->order_total - $result->discount_amount;
    //         $discount = $order->discount + $result->discount_amount;
    ...
    //         $this->sendSMS($sms, $phone);
    //     }
    //     return response()->json(['orderStatus'=>'Success','orderID'=>$order->id,'paymentType'=>'SSL','PaymentSatus'=>$result->status]);
...
public function recommend($amount=0){
        $data = Recommend::where('status',1)
            ->with('related')
 ...
 public function version_check(){
    ...
    public function validPaymentWithIPN($val_id)
    {
        $val_id = urlencode($val_id);
        $store_id = urlencode("Pizzahutbdlive");
        $store_passwd = urlencode("5FD89DA9E05DA80735");
        ...
        curl_setopt($handle, CURLOPT_SSL_VERIFYHOST, 2); # IF YOU RUN FROM LOCAL PC
