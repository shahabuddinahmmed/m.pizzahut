        ...
        if(Session::has('newCustomerOTP')){
            ...
            if($pin_code == $request->otp_pin){
                if(Session::has('oldCustomer')){
                    ...
                    if($customer){
                        HelperClass::updateOTPHistory($customer->phone_number);
                        Session::put('UserId',$customer->id);
                        Session::put('UserName','Customer');
                        Session::put('customer_mobile','88'.$customer->phone_number);
...
//                        return redirect()->route('/')->with('message','Thanks for your Login');
    ...
    public function customerLoginFormByMobile(Request $request){
        ...
        $count_otp_history = HelperClass::countOTPHistory($request->phone_number,$ip);
        if($count_otp_history<=3) {
...
//            return redirect()->route('register-customer')->with('message','First fill up the Registration Form');
            ...
            Session::put('message','Multiple times tries to login');
            return back();
        }
    }
    ...
    public function guestCustomerLoginFormByMobile(Request $request){
        ...
        $phone_number = substr($request->phone_number,-11);
        $ip = \Request::getClientIp(true);
        $count_otp_history = HelperClass::countOTPHistory($phone_number,$ip);
        if($count_otp_history<=5) {
            $customer = Customer::where('phone_number', 'like', '%' . $phone_number)->first();
            if ($customer) {
                Session::put('oldCustomer', $customer);
                $this->sendOtp($customer->phone_number);
...
//            return redirect()->route('register-customer')->with('message','First fill up the Registration Form');
            ...
            Session::put('otp_message','Multiple times tries to login');
            return back();
        }
    }
...
//            return $orders;
    ...
    protected function sendOTP($phone_number){
        $ip = \Request::getClientIp(true);
        $otp_count = HelperClass::countOTPHistory($phone_number,$ip);
        if($otp_count==true) {
            HelperClass::insertOTPHistory($phone_number,$ip);
            $digits = 6;
            $i = 0; //counter
            ...
            $param = "user=$user&pass=$pass&sms[0][0]= 88$parent_mobile &sms[0][1]=" . urlencode("Your OTP pin code is $pin_code
