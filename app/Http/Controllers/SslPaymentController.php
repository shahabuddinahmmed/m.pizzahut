    ...
    public function success(Request $request) {
        ...
        if($code == 200 && !( curl_errno($handle))) {
            ...
            if ($result->status === 'VALID' || $result->status=='VALIDATED') {
                ...
                $is_exist_ssl = SslPayment::where('val_id',$val_id)->first();
                if(!$is_exist_ssl) {
                    $ssl = new SslPayment();
                ...
                $v_result = $this->validPaymentWithIPN($val_id);
                $order_total = $order->order_total-$v_result->discount_amount;
                $discount    = $order->discount+$v_result->discount_amount;
                $order->is_active = 1;
                if($order->discount==0){
